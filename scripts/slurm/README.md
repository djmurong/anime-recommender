...
srun --partition=common --gres=gpu:1 --cpus-per-task=4 --mem=16G --time=00:15:00 --pty bash
source .venv/bin/activate
...
sbatch --partition=common --gres=gpu:1 scripts/slurm/04_train_two_tower.sh
sbatch --partition=common --gres=gpu:1 scripts/slurm/05_evaluate.sh
```
